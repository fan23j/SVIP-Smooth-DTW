...
def sample_frames(data_path, mode=None, num_clip=16, len_clip=1, augment=False):
    ...
    segments = np.linspace(0, len(all_frames) -
                           len_clip - 1, num_clip + 1, dtype=int)
    ...
    for j in range(eval_per_segment):
        ...
        for i in range(num_clip):
            ...
            start_index = segments[i] + \
                int((segments[i + 1] - segments[i]) / 4) * (j + 1)

            sampled_frames = []
            for i in range(len_clip):
                frame_index = start_index + i
                frame_path = os.path.join(
                    data_path, str(frame_index + 1) + '.jpg')
            ...
            if augment:
                ...
                if np.random.random() > 0.5:
                    transforms.append(
                        tf.RandomResizedCrop((180, 320), (0.7, 1.0)))
    ...
    with torch.no_grad():
        ...
        for i in tqdm(range(len(data_list))):
            ...
            for i in range(len(frames_list)):
                frames = frames_preprocess(
                    frames_list[i], cfg.MODEL.BACKBONE_DIM, cfg.MODEL.BACKBONE).to(device)
        ...
        img = cv2.resize(cv2.imread('square.png'), size,
                         interpolation=cv2.INTER_AREA)
        img = torch.tensor(img).unsqueeze(0).repeat(
            126, 1, 1, 1).permute(0, 3, 1, 2)
